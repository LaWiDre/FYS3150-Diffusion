...
class Diffusor_Generator:
    ...
    def _call_backend(self, script = "prototype2", algorithm = "ForwardEuler"):
        """
    ...
    def _gen_Diffusor_obj(self, M):
        ...
        """
        return Diffusor(M, self.T, self.dt, self.L, self.dx, "prototype2")
...
class Diffusor:
    ...
    def __init__(self, M, T, dt, L, dx, sim_type = "prototype2"):
        """
    ...
    def analytical_2D(self, t):
        pass
        # M0 = np.copy(self.M[0])
        # shape = tuple(list(M0.shape) + [len(M0.shape)])
    ...
    def animate_2D(self, filename = "animation_2D.mp4", frames = None):
        ...
        plt.xlabel("x [scaled]")
        plt.ylabel("y [scaled]")
        plt.title("Diffusion over Time, T= 0")

        # Creating the Color Meshes and Colorbar
        cax = ax.pcolormesh(x, y, self.M[0,:-1,:-1], vmin = np.min(self.M[:,:,1:-1]),
        vmax = np.max(self.M[:,:,1:-1]), cmap = 'magma')
    ...
    def plot_2D(self, t):
        ...
        # Plotting
        cax = plt.pcolormesh(x, y, self.M[idx,:-1,:-1], vmin = np.min(self.M),
        vmax = np.max(self.M), cmap = 'magma')
        cbar = fig.colorbar(cax)
        cbar.set_label("Intensity [scaled]")
        plt.show()
...
if __name__ == "__main__":
    ...
    dt = 1E-4                   # Time-Step
    L = (1, 2)                  # "Physical" Array Dimensions
    dx = 7E-2                   # Position-Step
